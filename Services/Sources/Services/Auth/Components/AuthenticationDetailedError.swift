...
public enum AuthenticationDetailedError: Error {
    ...
    /// Limit exceeded for the requested  resource
    case limitExceeded

    /// Amazon Cognito cannot find a multi-factor authentication method.
    case mfaMethodNotFound

    /// Software token TOTP multi-factor authentication is not enabled for the user pool.
