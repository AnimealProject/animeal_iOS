...
extension String {
    ...
    func getRangeWithOffsets(
        ...
        upperBoundOffset: Int
    ) -> Range<String.Index> {
        let sourceRangeLowerBoundDistance = self.distance(
            from: self.startIndex,
            to: sourceRange.lowerBound
        )
        let startIndex = self.index(
            self.startIndex,
            offsetBy: sourceRangeLowerBoundDistance + lowerBoundOffset
        )

        let sourceRangeUpperBoundDistance = self.distance(
            from: self.startIndex,
            to: sourceRange.upperBound
        )
        let endIndex = self.index(
            self.startIndex,
            offsetBy: sourceRangeUpperBoundDistance + lowerBoundOffset + upperBoundOffset
        )
    ...
    func findIndex(of element: Character, skipFirst: Int, startFrom: String.Index) -> String.Index {
        var copySkipFirst = skipFirst
        var elementIndex = startFrom
        while copySkipFirst > 0 && elementIndex < self.endIndex {
