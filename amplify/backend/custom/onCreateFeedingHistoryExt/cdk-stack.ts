...
export class cdkStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps, amplifyResourceProps?: AmplifyHelpers.AmplifyResourceProps) {
    ...
    const resolver = new appsync.CfnResolver(this, 'custom-resolver', {
      ...
      fieldName: 'createFeedingHistoryExt',
      typeName: 'Mutation', // Query | Mutation | Subscription
      requestMappingTemplate: appsync.MappingTemplate.fromFile(
        path.join(__dirname, '..', 'Mutation.createFeedingHistoryExt.req.vtl'),
      ).renderTemplate(),
      responseMappingTemplate: appsync.MappingTemplate.fromFile(
        path.join(__dirname, '..', 'Mutation.createFeedingHistoryExt.res.vtl'),
      ).renderTemplate(),
